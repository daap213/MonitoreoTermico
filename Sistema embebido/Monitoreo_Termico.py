...
def guardar_captura(ventana2,temp ="j"):
    global Datos
    ...
    count =  referencia() 
    if temp == "j":
        cv2.imwrite(Datos + "/image_" + str(count)+"_ref_", ventana2)
    else:
        cv2.imwrite(Datos + "/image_" + str(count) +"_Temp_ %.2f_.jpg"% temp, ventana2)
...
def verResultado():
    ...
    btnRadio1.configure(state="active")
    btnRadio2.configure(state="active")
    lblVideo.after(10, Enviar_telegram_desp)

def  Enviar_telegram_desp():
    global TempMax
